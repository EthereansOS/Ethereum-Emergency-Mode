if (typeof window !== 'undefined') {
    window.SolidityUtilities = function() {
        var compilersData;
        var compilers = {};
        var solcWorker;

        var giveMeThisScriptPath = async function giveMeThisScriptPath() {
            var thisText = window.SolidityUtilities.compile.toString().toLowerCase().split(' ').join('').split('\t').join().split('\r').join('').split('\n').join();
            var scripts = document.getElementsByTagName('script');
            for(var i in scripts) {
                var script = scripts[i];
                var text = script.innerText;
                if(script.src) {
                    text = await (await window.fetch(script.src)).text();
                }
                text = text.toLowerCase().split(' ').join('').split('\t').join().split('\r').join('').split('\n').join();
                if(text.indexOf(thisText) !== -1) {
                    return script.src;
                }
            }
            return null;
        ...
        var compileWork = function compileWork(compilerVersion, input) {
            if (!compilerVersion) {
                throw "Compiler Version needed";
            }
            if(compilers[compilerVersion]) {
                return JSON.parse(compilers[compilerVersion].compile(JSON.stringify(input)));
            }
            return new Promise(async function(ok, ko) {
                var id = compilerVersion + "_" + new Date().getTime();
                var enqueue = function() {
                    solcWorker.solcMessages[id] = [ok, ko]
                    solcWorker.postMessage({id, compilerVersion, input});
                };
                if(!solcWorker) {
                    solcWorker = {
                        solcMessages : {},
                        datas: [],
                        postMessage(data) {
                            solcWorker.datas.push(data);
                        }
                    };
                    enqueue();
                    var path = await giveMeThisScriptPath();
                    var serviceWorker = await navigator.serviceWorker.register(path);
                    await serviceWorker.unregister();
                    serviceWorker = undefined;
                    while(!serviceWorker || !serviceWorker.active) {
                        serviceWorker = await navigator.serviceWorker.register(path);
                    }
                    serviceWorker = serviceWorker.active;
                    serviceWorker.solcMessages = solcWorker.solcMessages;
                    serviceWorker.datas = solcWorker.datas;
                    solcWorker = serviceWorker;
                    navigator.serviceWorker.onmessage = navigator.serviceWorker.onmessage || function handler(event) {
                        var data = event.data;
                        if(!data.id) {
                            return;
                        }
                        var promise = solcWorker.solcMessages[data.id];
                        if (!promise) {
                            return;
                        }
                        delete solcWorker.solcMessages[data.id];
                        !data.output && data.script && !compilers[data.compilerVersion] && eval(data.script);
                        !data.output && data.script && !compilers[data.compilerVersion] && (compilers[data.compilerVersion] = window.solcWrapper(window["returnModule-" + data.compilerVersion]()))
                        data.output = data.output || JSON.parse(compilers[data.compilerVersion].compile(JSON.stringify(data.input)));
                        return data.error ? promise[1](data.error) : promise[0](data.output);
                    };
                    solcWorker.datas.forEach(it => solcWorker.postMessage(it));
                    return delete solcWorker.datas;
                }
                return enqueue();
            });
        };

        var cleanBytecode = function cleanBytecode(bytecode) {
            return (bytecode.substring(0, bytecode.length - 86) + bytecode.substring(bytecode.length - 22)).toLowerCase();
        };

        var compareBytecodes = function compareBytecodes(retrievedBytecode, compiledBytecodes) {
            retrievedBytecode = cleanBytecode(retrievedBytecode);
            var compareCompiled = function(compiled) {
                var keys = Object.keys(compiled);
                for (var i in keys) {
                    var key = keys[i];
                    if (retrievedBytecode === cleanBytecode(compiled[key].deployedBytecode)) {
                        return compiled[key];
                    }
                }
                return undefined;
            ...
            Object.keys(compiledBytecodes).forEach(key => {
                if(!comparation && (comparation = compareCompiled(compiledBytecodes[key]))) {
                    return;
                }
            });
            return comparation;
        ...
        return {
            ...
            async getCompilers() {
                return compilersData || (compilersData = JSON.parse(await (await fetch('//solc-bin.ethereum.org/bin/list.json')).text()));
            },

            retrieveBytecode(address) {
                return new Promise((ok, ko) => window.web3.eth.getCode(web3.utils.toChecksumAddress(address), (error, data) => error ? ko(error) : ok(data)));
            },

            async getSolcVersion(bytecode) {
                if (window.isEthereumAddress(bytecode)) {
                    bytecode = await this.retrieveBytecode(bytecode);
                }
                ...
                first = window.web3.utils.toDecimal("0x" + first);
                second = window.web3.utils.toDecimal("0x" + second);
                third = window.web3.utils.toDecimal("0x" + third);
                return (await this.getCompilers()).releases[first + "." + second + "." + third];
            },

            async compile(source, compilerVersion, optimization) {
                if (optimization == undefined || optimization == null) {
                    return {
                        optimized: await this.compile(source, compilerVersion, 200),
                        normal: await this.compile(source, compilerVersion, 0)
                    };
                }
                var input = {
                    language: 'Solidity',
                ...
                if (optimization !== undefined && optimization !== null && !isNaN(optimization) && optimization > 0) {
                ...
                var output = await compileWork(compilerVersion, input);
                if (output.errors) {
                    var error = '';
                    output.errors.map(it => error += (it.severity === 'warning' ? '' : (it.formattedMessage + '\n')));
                    error = (error && error.trim()) || error;
                    if (error) {
                        throw error;
                    }
                }
                output = output.contracts['Code.sol'];
                var result = {};
                Object.keys(output).map(key => {
                    result[key] = {
                        name: key,
                        abi: output[key].abi,
                        bytecode: '0x' + output[key].evm.bytecode.object,
                        deployedBytecode: '0x' + output[key].evm.deployedBytecode.object
                    };
                });
                return result;
            },
