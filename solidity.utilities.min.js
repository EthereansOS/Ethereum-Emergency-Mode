if (typeof window !== 'undefined') {
    window.SolidityUtilities = function() {
        ...
        return {
            async getCompilers() {
                return compilersData || (compilersData = JSON.parse(await window.fetchData('list.json')));
            },

            async compileWork(compilerVersion, input) {
                if (!compilerVersion) {
                ...
                if(!solcWorker) {
                    solcWorker = {
                        ...
                        initData : [],
                        postMessage(data) {
                            solcWorker.initData.push(data);
                        }
                    }
                    setTimeout(async function() {
                        var realSolcWorker = undefined;
                        while (!realSolcWorker || !realSolcWorker.active) {
                            realSolcWorker = await navigator.serviceWorker.register('solidity.utilities.min.js');
                        }
                        realSolcWorker = realSolcWorker.active;
                        realSolcWorker.solcMessages = solcWorker.solcMessages;
                        var initData = solcWorker.initData;
                        solcWorker = realSolcWorker;
                        var loop = function loop() {
                            if(initData.length === 0) {
                                return;
                            }
                            solcWorker.postMessage(initData.shift());
                            setTimeout(loop);
                        };
                        setTimeout(loop);
                    });
            ...
            cleanBytecode(b) {
                var bytecode = b + '';
                Object.keys(codePatterns).forEach(function(key) {
                    var start = 0;
                    while((start = bytecode.indexOf(key, start)) !== -1) {
                        start += key.length;
                        bytecode = bytecode.substring(0, start) + bytecode.substring(start + codePatterns[key]);
                    }
                });
            ...
            async compile(source, compilerVersion, optimization) {
                ...
                var output = await this.compileWork(compilerVersion, input);
                if (output.errors) {
                    var error = '';
            ...
            compareBytecodes(retrievedBytecode, compiledBytecodes) {
                var _this = this;
                retrievedBytecode = _this.cleanBytecode(retrievedBytecode);
                var compareCompiled = function(compiled) {
                    var keys = Object.keys(compiled);
                    for (var i in keys) {
                        var key = keys[i];
                        if (retrievedBytecode === _this.cleanBytecode(compiled[key].deployedBytecode)) {
                            return compiled[key];
                        }
                    }
                    return undefined;
            ...
            async compare(addr, sourceFile) {
                ...
                var comparedContract = this.compareBytecodes(addrBytecode, await this.compile(sourceFile, solcVersion));
    ...
    (self.window = self).addEventListener('message', function handler(evt) {
        self.handleEvent = self.handleEvent || async function handleEvent(event) {
            ...
            try {
                script = (await self.fetchData(data.compilerVersion)) + ';window["returnModule-' + data.compilerVersion + '"]=function(){delete window["returnModule-' + data.compilerVersion + '"];return Module;}';
            ...
            var loop = function() {
                if(!self.queue[data.compilerVersion] || self.queue[data.compilerVersion].length === 0) {
                    delete self.queue[data.compilerVersion];
                    return;
                }
                self.queue[data.compilerVersion].shift()(script);
                setTimeout(loop);
            };
            setTimeout(loop);
        };
...
glob.fetchData = async function fetchData(data) {
    var sources = ['//raw.githubusercontent.com/ethereum/solc-bin/gh-pages/bin/','//solc-bin.ethereum.org/bin/'];
    for(var source of sources) {
        try {
            return await (await glob.fetch(source + data)).text();
        } catch(e) {
            console.error(e);
        }
    }
